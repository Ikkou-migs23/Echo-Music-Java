...
public class TelaPlaylist extends JFrame {
    ...
    public TelaPlaylist(Playlist playlist) {
        ...
        scrollMusicas = criarListaMusicas();
        conteudo.add(scrollMusicas, BorderLayout.CENTER);
...
// --- músicas ---
    ...
    /** Reconstrói a lista de músicas exibida (chamado após editar uma música). */
    private void recarregarListaMusicas() {
        conteudo.remove(scrollMusicas);
        scrollMusicas = criarListaMusicas();
        conteudo.add(scrollMusicas, BorderLayout.CENTER);
        conteudo.revalidate();
        conteudo.repaint();
    ...
    private JPanel criarLadoDireitoLinha(Musica musica) {
        ...
        editar.addActionListener(e -> new TelaEditarMusica(musica, this::recarregarListaMusicas).setVisible(true));
